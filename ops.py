...
def conv2d(input, kernel, strides, padding, init_rate, name):
    with tf.compat.v1.variable_scope(name):
        W = tf.Variable(tf.random.normal(kernel, stddev=init_rate) * 0.01)
        b = tf.Variable(tf.random.normal([kernel[-1]], stddev=init_rate) * 0.01)
...
def batch_norm(x, name="batch_norm"):
    return tf.compat.v1.contrib.layers.batch_norm(x, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True)
